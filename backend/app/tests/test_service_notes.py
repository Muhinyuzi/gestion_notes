...
import pytest
from app.tests.conftest import TestingSessionLocal  # ✅ utilise la DB de test partagée
...
# ✅ Fake current user (marche avec les services)
class FakeAdmin:
    id = 1
    type = "admin"
    equipe = "Dev"
...
# ✅ Fixture DB qui utilise la même DB que les routes
@pytest.fixture
def db():
    db = TestingSessionLocal()

    # 👇 Ajoute un user test dans la vraie DB de test
    user = Utilisateur(
        id=1,
        nom="Admin",
